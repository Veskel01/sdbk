...
describe('DEFINE FUNCTION', () => {
  describe('Basic function definitions', () => {
    it('extracts function definition', () => {
      ...
      expectTypeOf<Schema['functions']['fn::greet']>().toExtend<{
        name: 'fn::greet';
      }>();
    });
  ...
  describe('Function parameters', () => {
    it('extracts function with typed arguments', () => {
      ...
      expectTypeOf<Fn['params'][0]['typeStr']>().toEqualTypeOf<'string'>();
    });
    ...
    it('extracts function with multiple typed arguments', () => {
      ...
      expectTypeOf<Fn['params'][1]['typeStr']>().toEqualTypeOf<'string'>();
      expectTypeOf<Fn['params'][2]['name']>().toEqualTypeOf<'out'>();
    });

    it('extracts function with optional parameters using option<T> type', () => {
      type Schema = ParseSchema<`
        DEFINE FUNCTION fn::last_option($required: number, $optional: option<number>)
      `>;

      type Fn = Schema['functions']['fn::last_option'];
      expectTypeOf<Fn['params'][0]['name']>().toEqualTypeOf<'required'>();
      expectTypeOf<Fn['params'][0]['typeStr']>().toEqualTypeOf<'number'>();
      expectTypeOf<Fn['params'][1]['name']>().toEqualTypeOf<'optional'>();
      expectTypeOf<Fn['params'][1]['typeStr']>().toEqualTypeOf<'option<number>'>();
    });
  });
