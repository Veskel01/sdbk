...
type _AccessBody<S extends string> =
...
type _ExtractBearerFor<S extends string> =
  Upper<S> extends `${string}FOR USER${string}`
    ? 'user'
...
type _ExtractJwtConfig<S extends string> =
  Upper<S> extends `${string}ALGORITHM ${string}`
...
type _ExtractAlgorithmFromOriginal<S extends string> =
  _ExtractAfterKeyword<S, 'ALGORITHM'> extends infer Alg extends string
...
type _ExtractSignup<S extends string> =
  Upper<S> extends `${string}SIGNUP ${string}`
    ? _ExtractExpressionAfterKeyword<S, 'SIGNUP'>
    : undefined;

type _ExtractSignin<S extends string> =
  Upper<S> extends `${string}SIGNIN ${string}`
    ? _ExtractExpressionAfterKeyword<S, 'SIGNIN'>
    : undefined;

type _ExtractRecordJwtConfig<S extends string> =
  Upper<S> extends `${string}WITH JWT${string}`
    ? {
...
type _ExtractKeyFromOriginal<S extends string> =
  Upper<S> extends `${string}WITH ISSUER${string}`
    ? _ExtractValueBeforeKeywordFromOriginal<S, 'KEY', 'WITH'>
    : _ExtractAfterKeyword<S, 'KEY'>;

type _HasWithRefresh<S extends string> =
  Upper<S> extends `${string}WITH REFRESH${string}` ? true : false;

type _ExtractRecordDuration<S extends string> =
  Upper<S> extends `${string}DURATION${string}`
    ? {
 ...
 */
type _ExtractAccessLevel<S extends string> =
 ...
 */
type _ExtractAuthenticate<S extends string> =
  Upper<S> extends `${string}AUTHENTICATE ${string}`
 ...
 */
type _ExtractFullDuration<S extends string> =
  Upper<S> extends `${string}DURATION${string}`
    ? {
 ...
 */
type _TrimToNextKeyword<S extends string> =
  _FindNextKeywordBoundary<S> extends infer Boundary
