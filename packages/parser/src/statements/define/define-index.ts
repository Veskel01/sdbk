...
type _ExtractModifiersAndName<S extends string> =
  Upper<S> extends `OVERWRITE ${infer Rest}`
...
type _AfterIndexName<S extends string> =
  Upper<FirstWord<S>> extends 'ON' ? _ExtractTable<AfterFirstWord<S>> : never;

// Handle optional TABLE keyword: ON [ TABLE ] @table
type _ExtractTable<S extends string> =
  Upper<FirstWord<S>> extends 'TABLE'
    ? _ExtractIndexFields<AfterFirstWord<S>>
    : _ExtractIndexFields<S>;

type _ExtractIndexFields<S extends string> =
  FirstWord<S> extends infer Table extends string
...
type _ExtractFieldNames<S extends string> =
  Upper<S> extends `${infer Fields} UNIQUE${string}`
...
type _IndexType<S extends string> =
  Upper<S> extends `${string}UNIQUE${string}`
    ? 'unique'
...
type _HasConcurrently<S extends string> =
  Upper<S> extends `${string}CONCURRENTLY${string}` ? true : false;
...
type _ExtractHnswConfig<S extends string> =
  Upper<S> extends `${string}HNSW ${infer Rest}`
    ? {
...
type _ExtractHnswDimension<S extends string> =
  Upper<S> extends `DIMENSION ${infer Rest}`
    ? ParseNumber<FirstWord<Rest>>
    : Upper<S> extends `${string} DIMENSION ${infer Rest}`
      ? ParseNumber<FirstWord<Rest>>
      : undefined;

type _ExtractHnswType<S extends string> =
  Upper<S> extends `${string}TYPE ${infer Rest}`
...
type _ExtractHnswDist<S extends string> =
  Upper<S> extends `${string}DIST ${infer Rest}`
...
type _ExtractHnswEfc<S extends string> =
  Upper<S> extends `${string}EFC ${infer Rest}` ? ParseNumber<FirstWord<Rest>> : undefined;

type _ExtractHnswM<S extends string> =
  Upper<S> extends `${string} M ${infer Rest}` ? ParseNumber<FirstWord<Rest>> : undefined;
