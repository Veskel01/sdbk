...
type _ConfigBody<S extends string> =
  _ExtractModifiersAndType<S> extends {
...
type _ExtractModifiersAndType<S extends string> =
...
type _GetConfigType<S extends string> =
  Upper<FirstWord<S>> extends 'API'
    ? { configType: 'api'; rest: AfterFirstWord<S> }
    : Upper<FirstWord<S>> extends 'GRAPHQL'
      ? { configType: 'graphql'; rest: AfterFirstWord<S> }
      : never;

type _ExtractMiddleware<S extends string> =
  Upper<S> extends `${string}MIDDLEWARE ${string}` ? _FindAndExtractMiddlewareList<S> : [];

type _FindAndExtractMiddlewareList<S extends string> =
  Upper<FirstWord<S>> extends 'MIDDLEWARE'
...
type _ExtractPermissions<S extends string> =
  Upper<S> extends `${string}PERMISSIONS ${string}`
    ? _FindAndExtractPermissionsValue<S>
    : undefined;

type _FindAndExtractPermissionsValue<S extends string> =
  Upper<FirstWord<S>> extends 'PERMISSIONS'
...
type _GetPermissionsValue<S extends string> =
...
type _ExtractTables<S extends string> =
  Upper<S> extends `${string}TABLES ${string}` ? _FindAndExtractTablesValue<S> : 'AUTO';

type _FindAndExtractTablesValue<S extends string> =
  Upper<FirstWord<S>> extends 'TABLES'
...
type _GetTablesValue<S extends string> =
  Upper<FirstWord<Trim<S>>> extends 'AUTO'
    ? 'AUTO'
...
type _ExtractFunctions<S extends string> =
  Upper<S> extends `${string}FUNCTIONS ${string}` ? _FindAndExtractFunctionsValue<S> : 'AUTO';

type _FindAndExtractFunctionsValue<S extends string> =
  Upper<FirstWord<S>> extends 'FUNCTIONS'
...
type _GetFunctionsValue<S extends string> =
  Upper<FirstWord<Trim<S>>> extends 'AUTO'
    ? 'AUTO'
    : Upper<FirstWord<Trim<S>>> extends 'NONE'
      ? 'NONE'
...
type _ExtractBracketedList<S extends string> =
  Trim<S> extends `[${infer Content}]${string}` ? `[${Content}]` : FirstWord<Trim<S>>;
