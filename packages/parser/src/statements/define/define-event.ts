...
type _EventBody<S extends string> =
...
type _ExtractTable<S extends string> =
  Upper<FirstWord<S>> extends 'ON' ? _ExtractTableName<AfterFirstWord<S>> : never;

type _ExtractTableName<S extends string> =
...
type _ExtractWhen<S extends string> =
  Upper<S> extends `${string}WHEN ${string}` ? _FindWhenClause<S> : undefined;
...
type _ExtractThen<S extends string> =
