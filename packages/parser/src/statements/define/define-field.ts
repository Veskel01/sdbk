...
type _FieldBody<S extends string> =
  ExtractNameAndModifiers<S> extends {
...
type _AfterFieldName<S extends string> =
  Upper<FirstWord<S>> extends 'ON'
...
type _ExtractComputed<S extends string> =
  Upper<S> extends `${string}COMPUTED ${string}` ? _FindAndExtractComputed<S> : undefined;
...
type _ExtractDefault<S extends string> =
  Upper<S> extends `${string}DEFAULT ${string}` ? _FindAndExtractDefault<S> : undefined;
...
type _ExtractDefaultValue<S extends string> =
  Upper<S> extends `ALWAYS ${string}` ? _ExtractAfterAlways<S> : ExtractExprUntilKeyword<S>;
...
type _HasDefaultAlways<S extends string> =
  Upper<S> extends `${string}DEFAULT ALWAYS ${string}` ? true : false;

// Extract VALUE clause (case-insensitive match, preserve original case)
type _ExtractValue<S extends string> =
  Upper<S> extends `${string}VALUE ${string}` ? _FindAndExtractValue<S> : undefined;
...
type _ExtractAssert<S extends string> =
  Upper<S> extends `${string}ASSERT ${string}` ? _FindAndExtractAssert<S> : undefined;
...
type _ExtractReference<S extends string> =
  Upper<S> extends `${string}REFERENCE${string}` ? { onDelete: _ExtractOnDelete<S> } : undefined;

type _ExtractOnDelete<S extends string> =
  Upper<S> extends `${string}ON DELETE REJECT${string}`
    ? 'REJECT'
...
type _ExtractType<S extends string> =
  _HasTypeKeyword<S> extends true ? ParseDataType<_TypeVal<S>> : unknown;

/** Check if string contains TYPE keyword (handles space, newline, tab after TYPE) */
type _HasTypeKeyword<S extends string> =
  _NormalizeWS<Upper<S>> extends `TYPE ${string}`
    ? true
...
type _TypeVal<S extends string> =
  _ExtractAfterType<S> extends infer R extends string ? _CleanType<Trim<R>> : never;
...
type _CleanType<S extends string> =
  Upper<S> extends `${string} READONLY${string}`
