...
type _ExtractSchemaMode<S extends string> =
  Upper<S> extends `${string}SCHEMAFULL${string}`
    ? 'schemafull'
...
type _ExtractTableType<S extends string> =
  Upper<S> extends `${string}TYPE ANY${string}`
    ? 'any'
...
// DROP detection
type _HasDrop<S extends string> =
  Upper<S> extends `DROP${string}`
    ? true
...
type _ExtractChangefeed<S extends string> =
  Upper<S> extends `${string}CHANGEFEED ${string}` ? _ParseChangefeedConfig<S> : undefined;
...
type _ExtractRelationConfig<S extends string> =
  Upper<S> extends `${string}TYPE RELATION${string}`
    ? {
...
type _ExtractRelationFrom<S extends string> =
  Upper<S> extends `${string}FROM ${string}`
    ? _GetRelationTable<S, 'FROM'>
    : Upper<S> extends `${string}IN ${string}`
      ? _GetRelationTable<S, 'IN'>
      : undefined;

type _ExtractRelationTo<S extends string> =
  Upper<S> extends `${string}TO ${string}`
