...
type _BucketBody<S extends string> =
...
type _ExtractBackend<S extends string> =
  Upper<S> extends `${string}BACKEND ${string}` ? _FindAndExtractQuoted<S, 'BACKEND'> : undefined;

type _ExtractPermissions<S extends string> =
  Upper<S> extends `${string}PERMISSIONS ${string}` ? _FindAndExtractPermissions<S> : undefined;

type _FindAndExtractPermissions<S extends string> =
...
type _ExtractPermissionsValue<S extends string> =
  Trim<S> extends `WHERE ${infer Expr}`
