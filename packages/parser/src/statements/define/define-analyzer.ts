...
type _AnalyzerBody<S extends string> = _ExtractNameAndModifiers<S> extends {
...
type _ExtractFunction<S extends string> = Upper<S> extends `${string}FUNCTION ${string}`
  ? _FindAndExtractValue<S, 'FUNCTION'>
  : undefined;

type _ExtractTokenizers<S extends string> = Upper<S> extends `${string}TOKENIZERS ${string}`
  ? _FindAndExtractList<S, 'TOKENIZERS'>
  : [];

type _ExtractFilters<S extends string> = Upper<S> extends `${string}FILTERS ${string}`
  ? _FindAndExtractList<S, 'FILTERS'>
  : [];
