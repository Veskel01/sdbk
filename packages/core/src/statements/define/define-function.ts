...
/**
 * Represents a parsed function parameter with name and type.
 */
export interface FunctionParam<Name extends string = string, Type = unknown> {
  name: Name;
  type: Type;
}
...
type _ParseFunction<S extends string> = Trim<S> extends `DEFINE FUNCTION ${infer Rest}`
...
type _ParseFunctionRest<S extends string> = Upper<S> extends `OVERWRITE ${string}`
...
type _QuickParseParams<S extends string> = Trim<S> extends ''
  ? []
...
type _ParseParam<S extends string> = Trim<S> extends `$${infer Name}: ${infer Type}`
  ? FunctionParam<Name, ParseType<Trim<Type>>>
  : Trim<S> extends `$${infer Name}:${infer Type}`
    ? FunctionParam<Name, ParseType<Trim<Type>>>
    : Trim<S> extends `$${infer Name}`
      ? FunctionParam<Name, unknown>
