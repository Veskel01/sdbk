...
type _ExtractBatch<S extends string> = Upper<S> extends `${string}BATCH ${infer Rest}`
  ? ParseNumber<FirstWord<Rest>>
  : undefined;

// Extract START clause
type _ExtractStart<S extends string> = Upper<S> extends `${string}START ${infer Rest}`
  ? ParseNumber<FirstWord<Rest>>
  : undefined;
